...
const TARGETS = [
  {
    name: '@chemx/o-command-palette',
    isScoped: true
  },
  {
    name: '@chem-x/o-command-palette',
    isScoped: true
  }
];
...
try {
  for (const target of TARGETS) {
    ...
    console.log(`\n\x1b[36m[o-command-palette] Publishing: ${target.name} (dry-run: ${isDryRun}${tag ? `, tag: ${tag}` : ''})\x1b[0m`);
...
console.log('\n\x1b[1m\x1b[36m--- o-command-palette Publish Summary ---\x1b[0m');
for (const res of results) {
  const icon = res.success ? '\x1b[32m✔\x1b[0m' : '\x1b[31m✕\x1b[0m';
  console.log(`  ${icon} ${res.name.padEnd(30)} ${res.success ? 'Published' : 'Failed'}`);
}
